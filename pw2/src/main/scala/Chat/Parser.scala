...
class Parser(tokensIt: Iterator[(String, Token)]):
  ...
  /** From the current token expected to be [[QUEL]] or [[COMBIEN]], parse a price
    * request sentence
    *
    * @return
  ...
  /** Parse the product expression from the sentence, can be either multiple
    ...
    *
    * This is the main method of the expression parsing, which uses the
    * precedence climbing algorithm to parse the expressions in a bottom-up
    * fashion, where the upper levels have higher precedence.
    *
    * @return
  ...
  /** Parse a logic expression from the sentence, if present. Otherwise, climb
    * one step up the precedence climbing algorithm. Logic expressions are OR
    * and AND binary expressions, each have no precedence between them, and
  ...
  /** Parses a product literal from the product token and an optional brand
    ...
    */
  // TODO: since we are somewhat typed here, should we ensure the product literal is typechecked directly?
  // TODO: i.e. there exists a known product with the given name and brand
