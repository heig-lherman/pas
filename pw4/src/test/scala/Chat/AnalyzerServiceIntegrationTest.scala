...
class AnalyzerServiceIntegrationTest extends FunSuite {
  ...
  override def beforeEach(context: munit.BeforeEach): Unit = {
    ...
    orderService = new OrderImpl(productService)
    session = new Session("test_session")
    analyzer = new AnalyzerService(productService, orderService, accountService)
  ...
  test("complex nested AND expressions should combine all products") {
    ...
    assertEquals(
      result,
      "Voici donc 1 croissant cailler, 1 croissant maison et 1 biere boxer! Cela coûte CHF 5.00 et votre nouveau solde est de CHF 25.00.",
    )
  }
}
