...
class AnalyzerServiceTest extends FunSuite {
  ...
  class SimpleProductService extends ProductService {
    ...
    override def produce(product: ProductName, brand: BrandName)(implicit
        ec: ExecutionContext,
    ): Future[Double] = ???
  }

  class SimpleOrderService(productService: ProductService)
      extends OrderService {
    override def resolvePrice(products: List[OrderService.Product]): Double = {
      products.foldLeft(0.0) { case (acc, (quantity, productName, brandName)) =>
        acc + (productService.getPrice(productName, brandName) * quantity)
      }
    }

    override def placeOrder(
        products: Seq[OrderService.Product],
  ...
  test("OR expression should select the cheaper option") {
    ...
    val orderService = new SimpleOrderService(productService)
    val analyzer =
      new AnalyzerService(productService, orderService, accountService)
    ...
    assertEquals(
      analyzer.reply(session)(leftCheaperExpr),
      "Cela coûte CHF 5.00.",
    )
    ...
    assertEquals(
      analyzer.reply(session)(rightCheaperExpr),
      "Cela coûte CHF 10.00.",
    )
  ...
  test("Default brand should be used when not specified") {
    ...
    val exprWithDefaultBrand =
      priceRequest(1 of "product1".product) // Should use brandA
    assertEquals(
      analyzer.reply(session)(exprWithDefaultBrand),
      "Cela coûte CHF 10.00.",
    )
  }
  ...
  test("Order should fail when user has insufficient balance") {
    ...
    val expensiveOrderExpr =
      orderRequest(1 of ("product1" by "brandB")) // Costs 15.0
  ...
  test("Product formatting should handle various numbers of products") {
    ...
    assert(
      threeResult.contains(
        "1 product1 brandA, 2 product1 brandB et 3 product2 brandC!",
      ),
    )
  }
